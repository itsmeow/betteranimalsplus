...
public class ClientLifecycleHandler {
    ...
    public void clientSetup(final FMLClientSetupEvent event) {
        ...
        R.addRender(EntityColossalSquid.class, 5F, r -> r.tSingle("squid_colossal").mSingle(new ModelColossalSquid<>()).simpleScale(e -> 1.9F).handleRotation((e, p) -> MathHelper.lerp(p, e.lastTentacleAngle, e.tentacleAngle)).applyRotations((e, a, y, p) -> {
        ...
        R.addRender(EntityGiantSquid.class, 3F, r -> r.tSingle("squid_giant").mSingle(new ModelGiantSquid<>()).simpleScale(e -> 2.2F).handleRotation((e, p) -> MathHelper.lerp(p, e.lastTentacleAngle, e.tentacleAngle)).applyRotations((e, a, y, p) -> {
            float f = MathHelper.lerp(p, e.prevSquidPitch, e.squidPitch);
            float f1 = MathHelper.lerp(p, e.prevSquidYaw, e.squidYaw);
            GlStateManager.translatef(0.0F, 0.5F, 0.0F);
            GlStateManager.rotatef(180.0F - y, 0.0F, 1.0F, 0.0F);

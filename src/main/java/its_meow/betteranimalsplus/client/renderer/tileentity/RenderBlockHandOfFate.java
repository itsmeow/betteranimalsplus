...
public class RenderBlockHandOfFate extends TileEntityRenderer<TileEntityHandOfFate> {
	...
	@Override
    public void render(TileEntityHandOfFate tileentity, double x, double y, double z, float partialTicks, int destroyStage) {
        ...
        if(tileentity.isOnFire() && this.i % 5 == 0) {
        	tileentity.getWorld().spawnParticle(Particles.FLAME, tileentity.getPos().getX() + ((this.rand.nextFloat() + 0.5F) / 2), tileentity.getPos().getY() + this.rand.nextFloat(), tileentity.getPos().getZ() + ((this.rand.nextFloat() + 0.5F) / 2), 0, 0.02F, 0);
        }

...
public class ModelWalrus extends ModelBase {
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entity) {
        ...
        if(entity instanceof EntityLivingBase) {
            this.head.rotateAngleX = (float) Math.toRadians(ModelBetterAnimals.getHeadPitch((EntityLivingBase)entity)) + 0.6981317007977318F;
            this.neck2.rotateAngleY = (float) MathHelper.clamp(Math.toRadians(ModelBetterAnimals.getHeadYaw((EntityLivingBase)entity)), Math.toRadians(-35F), Math.toRadians(35F));
        }
        if(!entity.isInWater()) {

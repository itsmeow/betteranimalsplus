...
public class ModelCrab<T extends LivingEntity> extends EntityModel<T> {
    ...
    public void render(T entity, float f, float f1, float f2, float f3, float f4) {
        RenderSystem.translated(0F, -0.05F, 0F);
        ...
        if(entity instanceof EntityCrab) {
            ...
            if(crab.getIsCrabRave() == 0) {
                ...
                this.rLeg03a.rotateAngleY = 0.45378560551852565F;
                this.lLeg03a.rotateAngleY = -0.45378560551852565F;
                float f3 = -(MathHelper.cos(f * 0.6662F * 2.0F + 0.0F) * 0.4F) * f1;
                float f4 = -(MathHelper.cos(f * 0.6662F * 2.0F + (float) Math.PI) * 0.4F) * f1;
                float f5 = -(MathHelper.cos(f * 0.6662F * 2.0F + ((float) Math.PI / 2F)) * 0.4F) * f1;
                float f6 = -(MathHelper.cos(f * 0.6662F * 2.0F + ((float) Math.PI * 3F / 2F)) * 0.4F) * f1;

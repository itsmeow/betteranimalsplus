...
public class EventHandlerClient {
	...
	/*
	 * @SubscribeEvent public void textureStitchEventPre(TextureStitchEvent.Pre
	 * event) { event.getMap().registerSprite(TextureRegistry.sparks);
	...
	public static void registerItemBlockModels(final ModelRegistryEvent event) {
		 ...
		 * initModel(BlockRegistry.handoffate, 0); // generics
		 * for(BlockGenericSkull skull : BlockRegistry.genericskulls.keySet()) {
		 * } //Items initModel(ItemRegistry.venisonRaw, 0);
		 * initModel(ItemRegistry.venisonCooked, 0);
		 * initModel(ItemRegistry.itemHirschgeistSkullWearable, 0);
		 * initModel(ItemRegistry.antler, 0); initModel(ItemRegistry.goatMilk,
		 * 0); initModel(ItemRegistry.goatCheese, 0);
	 ...
	 * ModelResourceLocation(item.getRegistryName(), "inventory")); } public
	 * static void initModel(Block block, int meta) {
	 * ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block),
	 * meta, new ModelResourceLocation(block.getRegistryName(), "inventory")); }
	 * public static void initModelOBJ(Block block, int meta) {
	 * ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block),
	 * meta, new ModelResourceLocation(block.getRegistryName() + ".obj",
	 * "inventory")); }
	 */

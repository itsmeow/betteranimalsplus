...
public class EntityLammergeier extends EntityTameableFlying implements IVariantTypes {
    ...
    protected SoundEvent getAmbientSound() {
        return this.isTamed() && this.dataManager.get(DATA_HEALTH_ID).floatValue() < 6.0F ? SoundEvents.ENTITY_PARROT_HURT : null;
    }
    ...
    static class AIMeleeAttack extends net.minecraft.entity.ai.EntityAIAttackMelee {
        ...
        public void updateTask() {
            ...
            // If the entity can reach its target and it's time to attack, reset the timer
            // and attack if the entity is not grabbed
            ...
            // If the entity is in range and entity is not grabbing a target and the
            // target's height is less than 3 blocks
            if (distanceToTarget <= reachToTarget && !entitylivingbase.isRiding() && entitylivingbase.height <= 3 && this.attackTick == 20) {
                ...
                // Remove targets of the entity (to avoid something stupid like a skeleton
                // shooting while being eaten by a bird)
            ...
            // If we've about reached the target lifting point and have a target grabbed, or
            // have completed movement, drop the entity
    ...
    static class AIRandomFly extends EntityAIBase {
        ...
        public void startExecuting() {
            ...
            if (random.nextInt(30) != 1 && !this.parentEntity.landedLast) {
                ...
                rPos = rPos.add(pos);
                /*
                 * double d0 = this.parentEntity.posX + (double)((random.nextFloat() * 2.0F -
                 * 1.0F) * 16.0F);
                 * double d1 = this.parentEntity.posY + (double)((random.nextFloat() * 2.0F -
                 * 1.0F) * 16.0F);
                 * double d2 = this.parentEntity.posZ + (double)((random.nextFloat() * 2.0F -
                 * 1.0F) * 16.0F);
        ...
        public EntityAIFindEntityNearestFlying(EntityLammergeier mobIn, Class<? extends EntityLivingBase> p_i45884_2_) {
            ...
            this.predicate = (@Nullable EntityLivingBase p_apply_1_) -> {
                ...
                if (p_apply_1_.isInvisible()) {
                    ...
                    return p_apply_1_.getDistance(EntityAIFindEntityNearestFlying.this.mob) > d0 ? false : EntityAITarget.isSuitableTarget(EntityAIFindEntityNearestFlying.this.mob, p_apply_1_, false, true);
                }
            };

...
public class EntityLammergeier extends EntityTameableFlyingWithTypes implements IVariantTypes<EntityTameableBetterAnimalsPlus> {
            ...
            if(!itemstack.isEmpty()) {
                if(itemstack.getItem().isFood()) {
                    ...
                    if(this.dataManager.get(EntityLammergeier.DATA_HEALTH_ID) < 20.0F) {
                        ...
                        for(int i = 0; i < 10; i++) {
                            this.world.addParticle(ParticleTypes.HAPPY_VILLAGER, posX + (this.getRNG().nextFloat() - 0.5F), posY + this.getRNG().nextFloat(), posZ + (this.getRNG().nextFloat() - 0.5F), 0, 0, 0);
                        }
    ...
    static class AIMeleeAttack extends Goal {
        ...
        public void tick() {
            ...
            if(attacker.getPassengers().size() == 0) {
                this.attacker.navigator.setPath(this.attacker.navigator.func_179680_a(entitylivingbase.getPosition(), 0), 0.4D);
            } else { // If the entity is grabbing a target, set it to move upwards
                this.attacker.navigator.setPath(this.attacker.navigator.func_179680_a(new BlockPos(targetX, this.liftY + 15, targetZ), 0), 0.4D);
            }
            ...
            if(attacker.getPassengers().size() == 0 && this.attacker.getEntityWorld().getBlockState(this.attacker.getPosition().up()).isNormalCube(world, this.attacker.getPosition().up())) {
                ...
                this.attacker.navigator.setPath(this.attacker.navigator.func_179680_a(rPos, 0), 0.4D);
            }
    ...
    static class AIRandomFly extends Goal {
        ...
        public boolean shouldExecute() {
            ...
            double d0 = move.getX() - this.parentEntity.posX;
            double d1 = move.getY() - this.parentEntity.posY;
            double d2 = move.getZ() - this.parentEntity.posZ;
        ...
        public void tick() {
            ...
            if(dist - lastDist < 0.05D) {
                ...
                if(timeSinceLastMove > 60) {
                    ...
                    parentEntity.getMoveHelper().setMoveTo(parentEntity.posX + parentEntity.getRNG().nextInt(3) - 1, parentEntity.posY + parentEntity.getRNG().nextInt(3) - 1, parentEntity.posZ + parentEntity.getRNG().nextInt(3) - 1, 0.5D);
                }
            } else {
                timeSinceLastMove = 0;
            }

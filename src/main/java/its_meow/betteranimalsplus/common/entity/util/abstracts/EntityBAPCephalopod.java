...
public abstract class EntityBAPCephalopod extends EntityWaterMobPathing {
    ...
    protected Vector3d getInkAngle(Vector3d vec) {
        Vector3d vec3d = vec.rotatePitch(this.prevSquidPitch * ((float) Math.PI / 180F));
    ...
    protected void squirtInk() {
        ...
        Vector3d vec3d = this.getInkAngle(new Vector3d(0.0D, -1.0D, 0.0D)).add(this.getPosX(), this.getPosY(), this.getPosZ());
        for(int i = 0; i < 30; ++i) {

...
public class EntityConfiguration {
	...
	public static void init(ForgeConfigSpec.Builder builder, EntityContainer container) {
		builder.comment(container.entityName);
		
		doRegister = builder.comment("If set to false, the entity is removed (will remove from existing worlds!)");
		
	}


	//public EntityConfiguration(Class<? extends Entity> entity, int min, int max, int weight, String[] biomesList) { 
	...
	public void loadSpawnValues(int weight, int min, int max, String[] biomesList) { 
		...
		//this.min = config.getInt("minGroup", this.categoryName, min, 1, 9999, "Must be greater than 0"); this.max = config.getInt("maxGroup",
		//this.categoryName, max, 1, 9999, "Must be greater or equal to min value!"); 

...
public class EntityCoyote extends EntityFeralWolf {
    ...
    protected SoundEvent getAmbientSound() {
        if((!this.isDaytime() || isHostileDaytime()) && !this.isTamed()) {
            ...
            return this.isTamed() && this.dataManager.get(DATA_HEALTH_ID) < TAMED_HEALTH / 2D ? SoundEvents.ENTITY_WOLF_WHINE : SoundEvents.ENTITY_WOLF_PANT;
        ...
        if(this.isTamed()) {
            if(!itemstack.isEmpty()) {
                if(itemstack.getItem().isFood()) {
                    ...
                    if(food.isMeat()
                    && this.dataManager.get(DATA_HEALTH_ID) < TAMED_HEALTH) {

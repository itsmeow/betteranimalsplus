...
public class ModelGoose<T extends LivingEntity> extends EntityModel<T> {
    ...
    public void setupAnim(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if(!entityIn.isInWater()) {
            this.rLeg01.xRot = MathHelper.cos(limbSwing * 0.6662F) * 1.4F * limbSwingAmount + 0.20943951023931953F;
            this.lLeg01.xRot = MathHelper.cos(limbSwing * 0.6662F + (float) Math.PI) * 1.4F * limbSwingAmount + 0.20943951023931953F;
        } else {
        ...
        if(entityIn instanceof PlayerEntity) {
            ...
            if((player.abilities.mayfly && player.abilities.flying) || (player != Minecraft.getInstance().player && player.hasImpulse)) {
                ...
                float rot = MathHelper.sin(ageInTicks * player.abilities.getFlyingSpeed() * 8F) * 0.5F;
            ...
            if(this.attackTime > 0.0F) {
                float f2 = MathHelper.sin((1.0F - (float) Math.pow(1.0F - this.attackTime, 3)) * (float) Math.PI);
                float f3 = MathHelper.sin(this.attackTime * (float) Math.PI) * 0.7F * 0.75F;

...
public class ModelLammergeier<T extends LivingEntity> extends EntityModel<T> {
    ...
    public void setupAnim(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (isFlying) {
            this.rWing01.zRot = MathHelper.cos(ageInTicks * 0.3F) * (float) Math.PI * 0.25F;
            ...
            if ((Math.abs(lammergeier.getDeltaMovement().y()) > 0 && (Math.abs(lammergeier.getDeltaMovement().x()) > 0.05 || Math.abs(lammergeier.getDeltaMovement().z()) > 0.05))
                ...
                if (Math.abs(lammergeier.getDeltaMovement().y() + lammergeier.lastMotionY) > 0.05 && lammergeier.getDeltaMovement().y() < 0) {
                    this.rWing01.zRot = MathHelper.cos(225 * 0.3F) * (float) Math.PI * 0.25F;
                }
            }
            this.body.xRot = (float) MathHelper.clampedLerp(lammergeier.lastRotX, lammergeier.rotX, limbSwing);
            ...
            this.rLeg01.xRot = MathHelper.cos(limbSwing * 0.6662F) * 1.4F * limbSwingAmount / f;
            this.lLeg01.xRot = MathHelper.cos(limbSwing * 0.6662F + (float) Math.PI) * 1.4F * limbSwingAmount / f;

        }

        if (lammergeier.isInSittingPose()) { // not actually sleeping, sitting
            this.head.xRot = 0;
        } else {

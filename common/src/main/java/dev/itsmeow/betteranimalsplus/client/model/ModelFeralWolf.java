...
public class ModelFeralWolf<T extends LivingEntity> extends ModelBetterAnimals<T> {
    ...
    public void setupAnim(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.head.xRot = (float) Math.toRadians(getHeadPitch(entity)) + 2.1816615649929116F;
        ...
        if (entity instanceof EntityFeralWolf) {
            ...
            if(wolf.isInSittingPose()) { // not actually sleeping, sitting
                sit = true;
        ...
        if(!sit) {
            ...
            this.lHindLeg01.xRot = MathHelper.cos(limbSwing * 0.8665F + (float) Math.PI) * swingModifier * limbSwingAmount - 0.22759093446006054F;
            this.rHindLeg01.xRot = MathHelper.cos(limbSwing * 0.8665F) * swingModifier * limbSwingAmount - 0.22759093446006054F;
            this.lForeleg01.xRot = MathHelper.cos(limbSwing * 0.8665F) * swingModifier * limbSwingAmount + 0.22759093446006054F;
            this.rForeleg01.xRot = MathHelper.cos(limbSwing * 0.8665F + (float) Math.PI) * swingModifier * limbSwingAmount + 0.22759093446006054F;
        }
    }

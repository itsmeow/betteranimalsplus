...
public class ModelFeralWolf<T extends LivingEntity> extends ModelBetterAnimals<T> {
    ...
    public void setRotationAngles(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.head.rotateAngleX = (float) Math.toRadians(getHeadPitch(entity)) + 2.1816615649929116F;
        this.head.rotateAngleY = netHeadYaw * 0.017453292F;
        boolean sit = false;

        if (entity instanceof EntityFeralWolf) {
            EntityFeralWolf wolf = (EntityFeralWolf) entity;
            this.tail01.rotateAngleX = ageInTicks + (wolf.isEntitySleeping() ? 1.7453292519943295F : 0.6981317007977318F);
            if(wolf.isEntitySleeping()) { // not actually sleeping, sitting
                sit = true;

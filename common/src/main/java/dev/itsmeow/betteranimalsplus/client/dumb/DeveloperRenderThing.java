...
public class DeveloperRenderThing {
    ...
    public static void playerRender(Player player, PlayerRenderer renderer, float partialTicks, PoseStack stack, MultiBufferSource buffers, int packedLight) {
        if (RENDER_INSTANCE == null) {
            RENDER_INSTANCE = new StupidRender(new EntityRendererProvider.Context(((EntityRendererAccessor) renderer).getEntityRenderDispatcher(), Minecraft.getInstance().getItemRenderer(), Minecraft.getInstance().getBlockRenderer(), ((EntityRendererAccessor) renderer).getEntityRenderDispatcher().getItemInHandRenderer(), Minecraft.getInstance().getResourceManager(), Minecraft.getInstance().getEntityModels(), renderer.getFont()));
        }

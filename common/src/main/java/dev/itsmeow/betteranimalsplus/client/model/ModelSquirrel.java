...
public class ModelSquirrel<T extends LivingEntity> extends EntityModel<T> {
    ...
    public void setupAnim(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        this.lArm01.xRot = MathHelper.cos(limbSwing * 0.6662F) * 1.4F * limbSwingAmount - 0.20943951023931953F;
        this.rArm01.xRot = MathHelper.cos(limbSwing * 0.6662F + (float) Math.PI) * 1.4F * limbSwingAmount - 0.20943951023931953F;
        this.rLeg01.xRot = MathHelper.cos(limbSwing * 0.6662F) * 1.4F * limbSwingAmount - 0.17453292519943295F;
        this.lLeg01.xRot = MathHelper.cos(limbSwing * 0.6662F + (float) Math.PI) * 1.4F * limbSwingAmount - 0.17453292519943295F;
        this.tail01.xRot = MathHelper.sin(limbSwing * 0.2F) * limbSwingAmount - (float) Math.toRadians(30);

        if (entityIn instanceof MobEntity) {
            this.neck.xRot = ModelBetterAnimals.getHeadPitch(entityIn) * 0.017453292F - 13;
            this.neck.yRot = ModelBetterAnimals.getHeadYaw(entityIn) * 0.017453292F * 0.5F;
            if (entityIn instanceof EntitySquirrel) {
                EntitySquirrel ent = (EntitySquirrel) entityIn;
                this.chest.xRot = ent.isBesideClimbableBlock() ? (float) Math.toRadians(-90) : 0.10471975511965977F;
            }
        }
    }
